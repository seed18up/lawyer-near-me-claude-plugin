...
// retry gaps จากรอบก่อน + ไฟล์ที่ยังต่ำ
const TARGET_FILES = [
  ...
  'law/tort-liability/tort-liability.json',
];
...
const LAW_NAMES = {
  ...
  'tort-liability.json':       'ละเมิดของเจ้าหน้าที่',
};

// query fallback เพิ่มเติมสำหรับรอบนี้
const EXTRA_PREFIXES = ['ฎีกา', 'คำพิพากษา', 'ศาลฎีกา'];

const LOG_FILE = 'fetch-dika-round5.log';
...
async function main() {
  log('=== fetch-dika-round5 เริ่ม ' + new Date().toISOString() + ' ===\n');
  ...
  for (const file of TARGET_FILES) {
    ...
    for (const s of data.sections) {
      ...
      const queries = [
        ...
        // fallbacks กว้างขึ้น
        EXTRA_PREFIXES[0] + ' ' + lawShort + ' ' + s.section,
        textSnip,
        thaiKw[0] + ' ' + thaiKw[1],
