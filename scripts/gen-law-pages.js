...
const categories = [
  ...
  { dir: 'law/tort-liability', json: 'tort-liability.json', emoji: '🏛️', cta: 'ละเมิดเจ้าหน้าที่รัฐ/ไล่เบี้ย', depth: 2 },
  { dir: 'law/jurisdictional-disputes', json: 'jurisdictional-disputes.json', emoji: '⚖️', cta: 'วินิจฉัยชี้ขาดอำนาจระหว่างศาล', depth: 2 },
  ...
  { dir: 'law/specialized-appeal-court', json: 'specialized-appeal-court.json', emoji: '🏛️', cta: 'ศาลอุทธรณ์คดีชำนัญพิเศษ', depth: 2 },
...
function buildPage(cat, data) {
  ...
  h += '    .card-footer{display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;padding-top:10px;border-top:1px solid var(--gray-200);margin-top:auto}\n';
  h += '    .card-law-name{font-size:12px;color:var(--text-muted)}\n';
  h += '    .cite-btn{font-size:11px;font-weight:700;color:var(--navy);background:var(--navy-light);border:none;padding:4px 10px;border-radius:var(--radius-sm);cursor:pointer;transition:background .15s;display:inline-flex;align-items:center;gap:4px}\n';
  h += '    .cite-btn:hover{background:#d0d9f0} .cite-btn.copied{background:#e8f5e9;color:#2e7d32}\n';
  h += '    .sec-ref{color:var(--navy);text-decoration:underline dotted;cursor:pointer}\n';
  h += '    .sec-ref:hover{text-decoration:underline}\n';
  ...
  h += 'function fmtText(raw){var clean=raw.replace(/\\[\\d+\\]/g,"");var inSub=false;return clean.split("\\n").map(function(ln){var t=ln.trim();if(!t)return"";t=t.replace(/ม\\.\\s*(\\d+(?:\\/\\d+)?(?:(?:ทวิ|ตรี|จัตวา|เบญจ))?)/g,function(m,n){return\'<a class="sec-ref" href="#" onclick="filterBySection(\\\'\'+(n.trim())+\'\\\');return false;">\'+m+"</a>";});if(/^\\(/.test(t)){inSub=true;return\'<p class="law-sub">\'+t+"</p>";}else if(inSub){return\'<p class="law-sub-cont">\'+t+"</p>";}else{return\'<p class="law-para">\'+t+"</p>";}}).join("");}';
  h += 'var citeText="มาตรา "+s.section+" ' + esc(shortName) + '";';
  h += 'var dikaHtml=(s.dika&&s.dika.length)?\'<div style="font-size:11px;color:var(--text-muted);margin-top:6px;">ฎีกา: \'+s.dika.slice(0,3).map(function(d){return\'<span style="background:var(--gray-100);padding:1px 6px;border-radius:4px;margin-right:4px;">\'+d+"</span>"}).join("")+"</div>":"";';
  h += 'var textHtml=s.text?\'<button class="card-text-toggle" onclick="toggleText(this)">📄 ดูข้อความกฎหมาย ▾</button><div class="card-text-body">\'+fmtText(s.text)+dikaHtml+\'</div>\':"";';
  ...
  h += '\'<div class="card-keywords">\'+kwHtml+\'</div><div class="card-footer"><span class="card-law-name">⚖️ ' + esc(shortName) + '</span><button class="cite-btn" onclick="copyCite(this,\\\'\'+citeText.replace(/\'/g,"\\\\\'")+\'\\\')">📋 คัดลอกอ้างอิง</button></div></div>\'}\n';
  h += 'function copyCite(btn,text){navigator.clipboard.writeText(text).then(function(){btn.textContent="✓ คัดลอกแล้ว";btn.classList.add("copied");setTimeout(function(){btn.textContent="📋 คัดลอกอ้างอิง";btn.classList.remove("copied")},2000)})}\n';
  ...
  h += 'function filterBySection(sec){lawGrid.style.display="";var cards=lawGrid.querySelectorAll(".law-card");var found=false;cards.forEach(function(card){var badge=card.querySelector(".section-badge");if(badge&&badge.textContent.replace("มาตรา ","").trim()===sec){card.classList.remove("hidden");if(!found){card.scrollIntoView({behavior:"smooth",block:"center"});found=true;}}else{card.classList.add("hidden")}});tocView.style.display="none";searchView.style.display="";resultInfo.style.display="";resultCount.textContent=found?1:0;searchTitle.innerHTML=\'มาตรา \'+sec+\' <a href="javascript:showAll()" style="font-size:13px;color:var(--navy);margin-left:12px;">← ดูทั้งหมด</a>\'}\n';
