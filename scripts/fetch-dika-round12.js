...
// round 12: ไฟล์ที่ยังมี 0 dika
const TARGET_FILES = [
  'law/magistrate-court/magistrate-court.json',
  'law/prosecutor/prosecutor.json',
  'law/specialized-appeal-court/specialized-appeal-court.json',
  'law/jurisdictional-disputes/jurisdictional-disputes.json',
  'law/bankruptcy-court/bankruptcy-court.json',
];

const LAW_NAMES = {
  'magistrate-court.json':         'ศาลแขวง คดีแขวง',
  'prosecutor.json':               'อัยการ พนักงานอัยการ',
  'specialized-appeal-court.json': 'ศาลอุทธรณ์คดีชำนัญพิเศษ',
  'jurisdictional-disputes.json':  'ขัดกันแห่งอำนาจศาล',
  'bankruptcy-court.json':         'ศาลล้มละลาย คดีล้มละลาย',
};

const LOG_FILE = 'fetch-dika-round12.log';
...
async function main() {
  log('=== fetch-dika-round12 เริ่ม ' + new Date().toISOString() + ' ===\n');
  let totalUpdated = 0, totalDika = 0;

  for (const file of TARGET_FILES) {
    let data;
    try { data = JSON.parse(fs.readFileSync(file, 'utf8')); }
    catch (e) { log('ERROR: ' + file + ' ' + e.message + '\n'); continue; }
